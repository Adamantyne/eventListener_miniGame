...
const paredeSuperior = document.querySelector(".parede-superior");
...
let texto3 = "Por exemplo, vamos supor que você queira criar uma div que, ao ser clicada, ela mude sua cor. A primeira coisa a se fazer é capturar essa div no js (com o querySelector, por exemplo). Caso você queira fazer com a propriedade do próprio objeto, basta você registrar o “onclick” (objeto.onclick= “função que altera a cor” )";
let texto4 ="Caso você queira fazer isso com o método addEventListener(), você deverá associa-lo ao objeto e, a priori, passar dois parâmetros. Você precisará indicar o evento e também o listener/função a ser executada (nesse caso, trabalha como uma função de call-back e possui como retorno em seu parâmetro o objeto 'Event'que carrega informações sobre o evento ocorrido).Voltando ao exemplo da div que muda de cor, ficaria: objeto.addEventListener(“click”, função(Event));, sendo a função passada como parâmetro responsável por alterar a cor da div.";
...
function adicionandoEventoAbrirPorta(){
    porta.addEventListener("click",()=>{
        ...
        paredeSuperior.classList.remove("escondido");
    })
}
...
function movendoMapa(valor){
    ...
    paredeSuperior.style.left=`${posicao+940}px`;
